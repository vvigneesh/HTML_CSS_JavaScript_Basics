...
function next(ele) {
    ...
    if (ele.code === "Enter") {
        //alert('Hi');
       document.getElementById('time-default1').focus() .click()
        //next.focus().click()
    }
}
function search(ele){
...
function timerCycle(){
    if (stoptime==false){
        ...
        preDefinedSecond=parseInt(document.getElementById('time-default1').value)
        document.getElementById('time-default').style.display="none";
        document.getElementById('time-default1').style.display="none";
        sec=sec+1;
        ...
        if (min>=preDefined && sec>=preDefinedSecond){
